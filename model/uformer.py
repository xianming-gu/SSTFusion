...
class Uformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        y = self.pos_drop(y)
        # Encoder
        conv0 = self.encoderlayer_0(y, mask=mask)  # 1 65536 16
        pool0 = self.dowsample_0(conv0)  # 1 16384 32
        conv1 = self.encoderlayer_1(pool0, mask=mask)  # 1 16384 32
        pool1 = self.dowsample_1(conv1)  # 1 4096 64
        conv2 = self.encoderlayer_2(pool1, mask=mask)  # 1 4096 64
        pool2 = self.dowsample_2(conv2)  # 1 1024 128
        conv3 = self.encoderlayer_3(pool2, mask=mask)  # 1 1024 128
        pool3 = self.dowsample_3(conv3)  # 1 256 256

        # Bottleneck
        conv4 = self.conv(pool3, mask=mask)  # 1 256 256
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, in_chans=1, dd_in=1, embed_dim=16, depths=depths,
                                win_size=8, mlp_ratio=4., token_projection='linear', token_mlp='leff', modulator=True,
                                shift_flag=False).to(device)
